...
function renderDetail() {
    ...
    detailContent.innerHTML = `
        ...
        <div class="detail-links-container" style="text-align: center; margin-top: 20px;">
            ${resource.youtubeLink ? `<a href="${resource.youtubeLink}" target="_blank" class="detail-link">${currentLang === 'zh' ? (resource.youtubeLinkNameZh || resource.youtubeLinkName) : resource.youtubeLinkName}</a>` : ''}
            ${resource.alternateLink ? `<a href="${resource.alternateLink}" target="_blank" class="detail-link">${currentLang === 'zh' ? (resource.alternateLinkNameZh || resource.alternateLinkName) : resource.alternateLinkName}</a>` : ''}
        </div>
    `;
